...
// ── API Call ─────────────────────────────────────────────────
// All requests go to the same-origin Vercel proxy at /api/gas.
// The proxy forwards them server-side to Google Apps Script,
// so the browser never makes a cross-origin request.
// This is what makes login work in MIT App Inventor WebViewer.
...
async function apiPost(params, timeoutMs = 30000) {
  ...
  // Primary: fetch to same-origin /api/gas — no CORS, no redirects in the browser
  try {
    return await _apiFetch(url, params, timeoutMs);
  } catch (fetchErr) {
    // Fallback: XHR (for very old Android WebView that lacks full fetch support)
    try {
      return await _apiXHR(url, params, timeoutMs);
    } catch (xhrErr) {
      const msg = fetchErr.message || xhrErr.message || 'Unable to connect to server. Please check your internet connection.';
      throw new Error(msg);
    }
  }
}

// fetch-based POST — primary method
function _apiFetch(url, params, timeoutMs) {
  return new Promise((resolve, reject) => {
    // AbortController is not available in very old Android WebView —
    // guard it so we fall through to XHR fallback instead of crashing.
    if (typeof AbortController === 'undefined') {
      reject(new Error('AbortController not supported'));
      return;
    }
    ...
    const timer = setTimeout(() => {
      ...
      settled = true;
      controller.abort();
      reject(new Error('Unable to connect to server. Request timed out.'));
    }, timeoutMs);
    ...
    fetch(url, {
      ...
      .then(text => {
        ...
        catch (e) { throw new Error('Invalid response from server. Please try again.'); }
        resolve(data);
      })
      .catch(err => {
        if (settled) return;
...
// XHR-based POST — fallback for old WebView
function _apiXHR(url, params, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = function () {
      ...
      if (xhr.status >= 200 && xhr.status < 400) {
        ...
        catch (e) { reject(new Error('Invalid response from server. Please try again.')); return; }
        resolve(data);
      } else {
    ...
    try {
      xhr.send('payload=' + encodeURIComponent(JSON.stringify(params)));
    } catch (e) {
      settled = true;
      clearTimeout(timer);
      reject(new Error('Unable to connect to server. Please check your internet connection.'));
...
// ── Session ──────────────────────────────────────────────────
// Saves to localStorage (primary) AND a session cookie (secondary).
// Cookie survives MIT App Inventor WebViewer's loadUrl() reload that
// wipes sessionStorage and can delay localStorage reads — which is what
// causes the "Password Manager Save → back to login" bug.
function saveSession(data) {
  const json = JSON.stringify(data);
  // 1. localStorage — primary, survives normal reloads
  try { localStorage.setItem(CONFIG.SESSION_KEY, json); } catch (_) {}
  // 2. sessionStorage — fast same-tab access
  try { sessionStorage.setItem(CONFIG.SESSION_KEY, json); } catch (_) {}
  // 3. Cookie — survives WebView loadUrl() hard reloads; expires with browser session
  try {
    const encoded = encodeURIComponent(json);
    document.cookie = CONFIG.SESSION_KEY + '=' + encoded + '; path=/; SameSite=Strict';
  } catch (_) {}
}

function _readCookieSession() {
  try {
    const match = document.cookie.match(
      new RegExp('(?:^|; )' + CONFIG.SESSION_KEY.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '=([^;]*)')
    );
...
function getSession() {
  // Try each storage in order — first non-null valid result wins
  try {
    const ls = localStorage.getItem(CONFIG.SESSION_KEY);
    if (ls) return JSON.parse(ls);
  } catch (_) {}
  try {
    const ss = sessionStorage.getItem(CONFIG.SESSION_KEY);
    if (ss) return JSON.parse(ss);
  } catch (_) {}
  return _readCookieSession();
}
...
function clearSession() {
  ...
  // Expire the cookie
  try {
    document.cookie = CONFIG.SESSION_KEY + '=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT; SameSite=Strict';
  } catch (_) {}
}
...
// ── App Init ─────────────────────────────────────────────────
// Guard flag — prevents initApp() from running more than once.
...
function initApp() {
  ...
  // Always hide the loading overlay when showing the app —
  // prevents it blocking all touches if it was left visible.
  hideLoading();
  ...
  // Register nav-link listeners only once — stacking them on every
  // initApp() call (from session retries) breaks touch on Android WebView.
  if (!_appInitialized) {
    _appInitialized = true;

    // Use event delegation on the sidebar nav instead of individual listeners.
    // Delegation survives DOM updates and works reliably on Android WebView.
    document.querySelector('.sidebar-nav').addEventListener('click', function (e) {
      const link = e.target.closest('.nav-link');
      if (!link) return;
      e.preventDefault();
      const mod = link.dataset.module;
      if (mod === 'users' && STATE.user && STATE.user.role !== 'Admin') {
...
async function loadDashboard() {
  ...
  try {
    ...
    if (res.success) {
      const d = res.data;
      $('statTotalProducts').textContent = d.totalProducts !== null && d.totalProducts !== undefined ? d.totalProducts : '0';
      $('statTotalStock').textContent    = d.totalStock    !== null && d.totalStock    !== undefined ? d.totalStock    : '0';
      $('statLowStock').textContent      = d.lowStock      !== null && d.lowStock      !== undefined ? d.lowStock      : '0';
      $('statOutOfStock').textContent    = d.outOfStock    !== null && d.outOfStock    !== undefined ? d.outOfStock    : '0';
      $('statTotalSuppliers').textContent = d.totalSuppliers     !== null && d.totalSuppliers     !== undefined ? d.totalSuppliers     : '0';
      $('statRecentStockIn').textContent  = d.recentStockInCount !== null && d.recentStockInCount !== undefined ? d.recentStockInCount : '0';
      $('statRecentStockOut').textContent = d.recentStockOutCount !== null && d.recentStockOutCount !== undefined ? d.recentStockOutCount : '0';
...
document.addEventListener('DOMContentLoaded', () => {
  // Ensure correct initial display regardless of HTML inline styles
  // ── Session boot ─────────────────────────────────────────
  // Sets login visible first (safe default), then immediately checks for a
  // saved session and shows the dashboard if one exists.
  // The retry loop handles Android WebView's occasional delay in making
  // localStorage readable at DOMContentLoaded time.
  $('loginPage').style.display = 'flex';
  $('mainApp').style.display = 'none';
  ...
  if (!bootSession()) {
    // Retry up to 5 times (250 ms apart) — covers Android WebView localStorage
    // not being immediately available at DOMContentLoaded.
    let retries = 0;
    const retryTimer = setInterval(function () {
      retries++;
      if (bootSession() || retries >= 5) {
        clearInterval(retryTimer);
        if (retries >= 5) bruteCheckOnLoad(); // no session found — restore lockout UI
      }
    }, 250);
  }

  // Re-check session whenever the page becomes visible again.
  // This covers the case where Google Password Manager "Save" causes a WebView
  // focus/reload cycle that would otherwise drop back to the login screen even
  // though localStorage still holds a valid session.
  document.addEventListener('visibilitychange', function () {
    if (document.visibilityState === 'visible' && $('mainApp').style.display === 'none') {
      bootSession();
    }
  });
...
function renderSalesSummary(data) {
  ...
  $('salesTodayCount').textContent    = data.todayCount    !== null && data.todayCount    !== undefined ? data.todayCount    : '0';
